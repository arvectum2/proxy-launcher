...
def test_repository_contract_pins_exact_accepted_0_2_4_candidate():
    contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
    assert contract["version"] == "0.2.4"
    assert contract["candidate_source_commit"] == "e2278dbbd99b0d98ba9e4f836e40b2d60ea94b30"
    assert contract["candidate"]["portable_zip_sha256"] == "467864f286dc4c4de6c0f6033ae5cd5420a691c1bc9a32bf4891285c8c1d29bf"
    assert contract["candidate"]["setup_sha256"] == "28eb0b06c2f478b46d5845c6bb1970c96e20a2f6fdaf1d48501ed69f52ea6965"
    assert contract["candidate"]["application_sha256"] == "0415226f882e16a0ce370b766c4c68d3c23861f97361da093a1fd9d9f0e0832c"
    assert contract["predecessor"]["setup_sha256"] == "5808bde9d0ac45048d50bc256878519257f53bf0a9fa523a81ccb2eff0e21414"
    assert contract["predecessor"]["application_sha256"] == "f8d98f987ce92dee7979b12b69a56d120ddb12244bebe2559bc51359a53f9c7a"


def test_build_evidence_requires_exact_product_bytes_and_all_physical_gates(tmp_path):
    ...
    assert evidence["scope"] == "EXACT_PRODUCT_SET_READY_FOR_REL011_SIGNING"
    assert evidence["release_assets"]["setup"]["sha256"] == sha(b"setup")
    assert evidence["release_assets"]["portable_zip"]["sha256"] == sha(b"portable")
    assert evidence["lifecycle"]["gates"]["upgrade"] == "PASS"
    assert evidence["lifecycle"]["gates"]["rollback"] == "PASS"
    assert evidence["lifecycle"]["app_control"]["code_integrity_3077"] == "PASS"
    assert evidence["lifecycle"]["code_integrity_3077_count"] == 0
    assert evidence["signed_set_binding"]["rel013_post_sign_binding_required"] is True
    assert evidence["signed_set_binding"]["final_signed_set_pass_claimed"] is False


def test_tampered_release_asset_fails_closed(tmp_path):
    module, contract, candidate, physical, release, setup = make_fixture(tmp_path)
    setup.write_bytes(b"tampered")
    with pytest.raises(module.EvidenceError, match="setup release asset SHA-256 mismatch"):
        module.build_evidence(contract, candidate, physical, release)


def test_blocked_or_partial_physical_gate_fails_closed(tmp_path):
    module, contract, candidate, physical_path, release, _ = make_fixture(tmp_path)
    physical = json.loads(physical_path.read_text(encoding="utf-8"))
    physical["gates"]["repair"] = "BLOCK"
    physical_path.write_text(json.dumps(physical), encoding="utf-8")
    with pytest.raises(module.EvidenceError, match="repair"):
        module.build_evidence(contract, candidate, physical_path, release)


def test_code_integrity_block_fails_closed(tmp_path):
    module, contract, candidate, physical_path, release, _ = make_fixture(tmp_path)
    physical = json.loads(physical_path.read_text(encoding="utf-8"))
    physical["code_integrity_3077_count"] = 1
    physical_path.write_text(json.dumps(physical), encoding="utf-8")
    with pytest.raises(module.EvidenceError, match="3077"):
        module.build_evidence(contract, candidate, physical_path, release)
