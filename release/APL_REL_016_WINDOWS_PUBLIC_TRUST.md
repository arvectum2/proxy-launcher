...
**Decision date / current-requirements review:** 2026-09-19
**First eligible release:** `0.2.11+`
...
`v0.2.5`, `v0.2.9`, and the now-published `v0.2.10` remain immutable. Their governed release evidence is valid, but production Windows public Authenticode signing is not active on those published releases. Every embedded-signature byte change under APL-REL-016 therefore belongs to `0.2.11+`.
...
The MinDigital draft dated **2026-08-28**, re-checked on **2026-09-19**, is important: it defines a separate **RSA code-signing certificate using international cryptographic algorithms**, requires `codeSigning` EKU, and references the CAB Forum code-signing policy OID `2.23.140.1.4.1`.
...
- mutating `v0.2.5`, `v0.2.9`, or `v0.2.10` or replacing their assets;
...
Production public trust remains pending until a `0.2.11+` candidate has an eligible production identity and physical evidence equivalent to:
