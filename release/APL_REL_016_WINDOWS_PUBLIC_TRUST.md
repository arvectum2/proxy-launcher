...
`v0.2.5` and the now-published `v0.2.6` remain immutable. Their governed release evidence is valid, but production Windows public Authenticode signing is not active on either published release. Every embedded-signature byte change under APL-REL-016 therefore belongs to `0.2.7+`.
...
- mutating `v0.2.5` or `v0.2.6` or replacing their assets;
...
Production public trust remains pending until a `0.2.7+` candidate has an eligible production identity and physical evidence equivalent to:
