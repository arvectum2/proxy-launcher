...
source change
  -> Pull Request
  -> main
  -> green CI (exact-main Windows + Astra DEB + RED OS RPM + AppImage + macOS cloud packaging)
  -> trusted Mac mini Developer ID signing + Apple notarization for both macOS architectures
  -> version consistency and release-evidence validation
  -> Git tag (vX.Y.Z)
  -> GitHub Release workflow (.github/workflows/release.yml)
  -> canonical Windows tag builds + reuse of exact-main Astra/Linux DEB, RED OS RPM, AppImage and notarized macOS DMGs
...
* **Generic Linux x86_64 tarball:** `Arvectum-Proxy-Launcher-X.Y.Z-linux-x86_64.tar.gz` (development/future distribution track; not part of the current public release set)
...
* **macOS (production distribution):** Engineering, packaging and real-host functional acceptance are complete on Apple Silicon, including recovery/autostart, Safari/CFNetwork routing hardening and native Aqua UI. Exact-main Apple Silicon and Intel cloud builds are promoted only through the trusted Mac mini production-signing lane. Public DMGs are signed by `Developer ID Application: LLC ARVECTUM (VML75VY94V)`, use Hardened Runtime and secure Apple timestamps, are notarized by Apple with the canonical `Arvectum Release Bot`, have stapled tickets, and must pass Gatekeeper before they are eligible for release publication. Raw ad-hoc cloud DMGs remain QA inputs and are never published directly.
