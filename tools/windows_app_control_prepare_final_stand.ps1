...
if ($BasePolicyId -ne $ExpectedBasePolicyId) { throw "This governed ARVECTUM-DEMO preparation is bound to base policy $ExpectedBasePolicyId." }
if (Test-Path -LiteralPath $RunRoot) { throw "RunRoot already exists; refusing to overwrite prior stand evidence: $RunRoot" }
...
if ($hasHistoricalPackage -xor $hasHistoricalQa) { throw 'Historical stand-mode recovery requires BOTH package ZIP and QA evidence paths.' }
if (-not $sourceCommit -and -not $hasHistoricalPackage) { throw 'No usable Git working copy is available; supply both exact historical files for stand-mode baseline recovery.' }

New-Item -ItemType Directory -Path $RunRoot -Force | Out-Null

Write-Host '=== 1/3 Recover exact historical 0.2.2 P0.4 baseline ==='
$recoverArgs = @{ RepositoryRoot = $RepositoryRoot; OutputDirectory = $baselineReleaseDir; EvidenceDirectory = $baselineRecoveryDir }
if ($hasHistoricalPackage) { $recoverArgs.HistoricalPackageZipPath = $HistoricalPackageZipPath; $recoverArgs.HistoricalQaEvidencePath = $HistoricalQaEvidencePath }
& $recover @recoverArgs
...
if ([string]$baselineManifest.result -ne 'PASS' -or [string]$baselineManifest.source.commit -ne $ExpectedLegacyCommit -or [string]$baselineManifest.source.git_blob_sha1 -ne $ExpectedLegacyBlobSha1) { throw 'Recovered baseline does not match the governed historical identity.' }
...
if ([string]$baselineTrust.result -ne 'PASS' -or ([Guid]([string]$baselineTrust.base_policy_id)) -ne $BasePolicyId) { throw 'Baseline trust pack is not a PASS record for the canonical base policy.' }
...
if ([string]$currentTrust.result -ne 'PASS' -or [string]$currentTrust.mode -ne 'ReferenceFullHash' -or ([Guid]([string]$currentTrust.base_policy_id)) -ne $BasePolicyId) { throw 'Current trust pack is not a ReferenceFullHash PASS record for the canonical base policy.' }
...
$state = [ordered]@{
    schema = 'arvectum.proxy.apl-win-014-final-stand-state.v1'; task = 'APL-WIN-014'; prepared_utc = [DateTime]::UtcNow.ToString('o')
...
Set-Content -LiteralPath (Join-Path $RunRoot 'SHA256SUMS.txt') -Encoding ASCII -Value @("$stateHash  stand-state.json","$deploymentHash  POLICIES_TO_DEPLOY.txt")
