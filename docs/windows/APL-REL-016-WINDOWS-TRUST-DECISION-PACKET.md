...
Date / current-requirements review: `2026-09-25`
Repository: `arvectum2/proxy-launcher`
Issue: `#30`
Release boundary: `v0.2.5`, `v0.2.9`, and the published `v0.2.16` are immutable; any production Authenticode/public-trust byte change belongs to `v0.2.17+`.
...
The current public `v0.2.16` Windows installer is governed and byte-identified but unsigned at the PE/Authenticode layer; `v0.2.5` remains the historical first sealed Windows baseline. The existing CryptoPro/Rutoken detached signature remains a separate Russian release-evidence layer and must not be described as Microsoft-native publisher trust.

## 2. Current Microsoft trust model

As of 2026-09-25, Microsoft documents three materially different concepts that must remain separate:
...
|---|---|---|---|---|
| Public direct download, unsigned | Exact bytes may still be governed/reproducible | Artifact identity only | Publisher identity, Smart App Control signing trust, SmartScreen reputation | Not acceptable as the long-term `0.2.17+` public trust target |
| Public direct download, RSA Authenticode from Microsoft-trusted provider | Valid signature over installer and relevant executable payloads; trusted chain; timestamping policy | Native Windows publisher identity and signing integrity; compatible basis for Smart App Control | Guaranteed first-download SmartScreen reputation | Primary technical candidate for direct-download `0.2.17+` |
...
For a future direct-download `0.2.17+` release, the lowest-surprise Windows-native architecture is:
...
## 8. Acceptance matrix for `0.2.17+`
...
| Immutability | `v0.2.5` and `v0.2.9` tags/assets remain untouched; all production-signing changes are in `0.2.17+` |
...
- prefer an RSA Microsoft-trusted signing path for `0.2.17+` direct downloads;
...
- do not mutate `v0.2.5` or `v0.2.9`.
...
6. `0.2.17+` release acceptance matrix;
