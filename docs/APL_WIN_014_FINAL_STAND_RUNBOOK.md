...
The production installer maintenance scripts are not sourced from the production portable ZIP. The canonical `v0.2.3-ru.2` Inno Setup definition embeds them from the governed release source tree. App Control authoring therefore requires an explicit `MaintenanceSourceRoot` containing the exact release-source `upgrade_helper.ps1` and `uninstall_helper.ps1` bytes from release-policy commit `47823585c42da54ab51dc2246583dc24d74d4ba6`. They must match these immutable identities:

- `upgrade_helper.ps1`: `77e8bcb4d27aad5b2d1b40753f3ec2dfa2419e48a07f2eb17a7b15f2a9232218`
- `uninstall_helper.ps1`: `7abc1fe332975440d2c84be608773a890c5bb4deb130eea54378a128e79b0a44`

The portable ZIP remains independently hash-pinned for the portable application identity, but it is **not** treated as a maintenance-helper source.
...
- exact governed maintenance-source directory containing the two release-source helper scripts above;
...
Run from elevated Windows PowerShell 5.1 in the repository root, supplying the exact signing evidence and exact maintenance source:
...
& .\tools\windows_app_control_prepare_final_stand.ps1 `
    ...
    -MaintenanceSourceRoot 'C:\Arvectum\Evidence\APL-WIN-014\release-maintenance-source\v0.2.3-ru.2'
```

The script fails closed if the signing evidence differs from the canonical `67d379...` identity or either maintenance helper differs from its fixed sealed SHA-256. It records `russian_release_provenance = PREVERIFIED_EXACT_HASH_BOUND`, `local_cryptopro_verification = NOT_REQUIRED`, and the sealed maintenance-source contract; it never converts an absent local CryptoPro verification into a false local PASS.
...
2. current `0.2.3` `ReferenceFullHash` supplemental, including the exact statically-derived Inno Setup 6.7.1 child runtime and the exact release-source maintenance scripts.
...
Any identity mismatch, unexpected file, missing/unauthorized policy, Audit mode, occupied governed port, changed release byte, signing-evidence drift, maintenance-helper drift, or incomplete final sub-gate is **BLOCK**, not a reason to weaken Windows protection or substitute another binary. Preserve the failed run directory as evidence and use a new `-RunRoot` for any deliberate rerun.