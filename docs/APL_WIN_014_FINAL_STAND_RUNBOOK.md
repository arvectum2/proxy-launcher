...
Any identity mismatch, unexpected file, missing/unauthorized policy, Audit mode, occupied governed port, changed release byte, or incomplete final sub-gate is **BLOCK**, not a reason to weaken Windows protection or substitute another binary. Preserve the failed run directory as evidence and use a new `-RunRoot` for any deliberate rerun.
